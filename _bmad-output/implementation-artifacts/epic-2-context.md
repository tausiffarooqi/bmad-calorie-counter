...
Users can submit an Entry — by photo or free-text description, from the same logging screen — and receive a calorie estimate that gets stored as a permanent log record. The system runs vision recognition on photos (generating a text description first, then estimating from that), or estimates directly from typed text. If input is too vague to estimate reliably, the system never guesses — it asks the user to retry with more detail. Every Entry is timestamped on submission, and for photos, only the generated description is retained; the photo itself is discarded right after analysis. For a photo Entry, the user also sees a live preview of the exact photo they uploaded, staying visible continuously across the preparing, estimating, success, and retry states of the same Log Photo dialog. On a successful photo estimate, the dialog no longer auto-closes — it stays open, showing the photo preview alongside the estimated calories, until the user explicitly closes it via the dialog's own existing close control; the text-logging dialog's auto-close-on-success behavior is unchanged. Either way, the Entries list and Remaining Calorie Budget refresh immediately once the Entry is durably saved, regardless of whether the photo dialog has been closed yet. The preview itself is discarded (never persisted or sent anywhere new) the moment the dialog closes. This epic is complete and demoable on its own: it records what was eaten and its estimated calories, without yet computing a running budget or issuing recommendations (that's Epic 3).
...
- On a successful photo estimate, the Log Photo dialog does not auto-close: the photo preview and the calorie estimate both remain on screen until the user explicitly closes the dialog themselves (its existing close control — no new UI element is introduced for this). This is specific to the photo dialog; the text-logging dialog's existing auto-close-after-success behavior is unchanged.
- Regardless of whether the photo dialog has been closed yet, the Entries list and Remaining Calorie Budget must refresh immediately once the estimate succeeds — the Entry is already durably saved at that point, and only the dialog's own closing timing is deferred to the user, not the rest of the app reflecting the new Entry.
- The photo preview is scoped strictly to the current submission attempt: never persisted, never sent to any new server endpoint, and discarded along with the rest of the dialog's local state as soon as the dialog closes (by explicit user action on the photo path; by either auto-close or user action on the text path). It must never surface in the Entries list, after a reload, or on another device. This is purely an additive client-side UX behavior — it does not change FR-6/AD-4's rule that the photo is discarded server-side after analysis and never persisted.
...
- The shared `useEntrySubmission` hook (consumed identically by the text dialog since Stories 2.1–2.3 and by the photo dialog since Story 2.2) previously auto-closed its host dialog on a successful estimate in both cases. It now needs to support two distinct behaviors from one shared implementation: the text dialog keeps auto-closing on success (unchanged), while the photo dialog must not auto-close on success — it only closes when the user acts on the dialog's own close control. Whatever mechanism achieves this (e.g. a caller-supplied option/flag), the rest of the hook's state machine (submitting → success/insufficient_detail/error, staleness/abort guarding, `onSuccess` callback triggering the list/budget refresh) stays identical between the two callers — only the auto-close-on-success behavior diverges.
- The `onSuccess` refresh callback (bumping the Entries list / Remaining Calorie Budget's refresh key) must still fire as soon as the estimate succeeds, independent of the auto-close change — the list/budget refresh is not gated on the dialog closing, only the dialog's own close timing is.
...
- Photo dialog success state: the "Logged — about N calories" confirmation renders alongside the still-visible photo preview and stays on screen — no auto-dismiss timer — until the user taps the dialog's existing close affordance (its close control, Escape, or an overlay click). The text dialog's success confirmation still auto-closes as before; this is a deliberate, photo-only divergence, not a general pattern change.
...
- Stories 2.1–2.3 (text dialog) and Story 2.6 (photo dialog) share the same `useEntrySubmission` hook end to end. That hook's auto-close-on-success behavior now must diverge by caller: unchanged (auto-close) for the text dialog, removed (stays open until explicit close) for the photo dialog — while every other piece of shared behavior (submission state machine, staleness/abort handling, the `onSuccess` refresh trigger) stays identical between the two. Any change to the hook needs to preserve this per-caller distinction rather than re-coupling the two dialogs' close behavior.
- The Entries list / Remaining Calorie Budget refresh triggered by `onSuccess` is not gated on the photo dialog being closed — it must fire on estimate success exactly as it does for the text dialog, even while the photo dialog remains open showing the preview and estimate.
